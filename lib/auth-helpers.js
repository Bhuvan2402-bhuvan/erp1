 ...
 * Wrapped in React cache() for zero-overhead per-request memoization across components.
 */
  ...
  try {
    ...
    if (!dbUser) {
      try {
        ...
        const sessionRole = headersList.get('x-user-role') || cookieList.get('x-user-role')?.value;

        if (sessionEmail || sessionUid || sessionRole) {
          const cleanEmail = sessionEmail ? sessionEmail.toLowerCase() : '';
          
          if (cleanEmail || sessionUid) {
            dbUser = await prisma.user.findFirst({
              where: {
                OR: [
          ...
          // Fallback object if record isn't in DB yet
          if (!dbUser && (cleanEmail || sessionUid || sessionRole)) {
            const fallbackEmail = cleanEmail || 'user@erp.com';
            const computedRole = sessionRole || (fallbackEmail.includes('admin') ? 'ADMIN' : fallbackEmail.includes('faculty') ? 'FACULTY' : 'STUDENT');
            const uid = sessionUid || ('usr-' + fallbackEmail.replace(/[^a-z0-9]/g, ''));
            const name = computedRole === 'ADMIN' ? 'NSS Lead Admin' : computedRole === 'FACULTY' ? 'Faculty Coordinator' : 'NSS Volunteer';
