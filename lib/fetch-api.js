 ...
 *
 * Routing logic:
 *  - If the endpoint starts with "/api/..." AND `NEXT_PUBLIC_API_URL` is set,
 *    the request is forwarded to the Render Express backend with a Supabase
 *    Bearer token attached.
 *  - If `NEXT_PUBLIC_API_URL` is not set (local dev, Vercel-only mode), the
 *    request falls through to the Next.js API routes at `/api/...`.
 *  - Absolute URLs (http/https) are called as-is with the Bearer token.
 *
 * Usage:
 *   import { apiFetch } from '@/lib/fetch-api';
 *
 *   // Calls Render backend (or Next.js API in dev):
...
function resolveUrl(endpoint) {
  ...
  // No NEXT_PUBLIC_API_URL — fall through to Next.js API routes
  return endpoint;
}
