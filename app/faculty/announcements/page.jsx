    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className={`${cardClass} p-6`}>
        <div className="flex gap-3">
          <input value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Post an announcement..." className="flex-1 px-4 py-2 border rounded-lg text-sm dark:bg-slate-700 dark:border-slate-600 dark:text-white" onKeyDown={e => e.key === 'Enter' && handlePostPublicMessage()} />
          <button onClick={handlePostPublicMessage} className="px-4 py-2 bg-gradient-to-r from-logo-navy to-logo-teal text-white rounded-lg hover:opacity-90 text-sm font-medium transition shadow-sm">Post</button>
        </div>
      </div>
