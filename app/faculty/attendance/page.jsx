  ...
  const fetchAttendance = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error('Failed to load branch attendance');
      }
    } catch (err) {
      toast.error('Network error loading attendance');
    }
    setLoading(false);
  }, [search, selectedEvent, statusFilter]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xs">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            ...
            Branch Attendance Register
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Track student volunteer event attendance and service hours in your department.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <Download className="w-4 h-4" /> Export Branch Register
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 flex items-center gap-4 shadow-xs">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Branch Attendance Logged</p>
      ...
      {/* Filter Toolbar */}
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 shadow-xs">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-xs">
        {loading ? (
          <div className="p-12 text-center text-slate-500 animate-pulse">Loading branch attendance records...</div>
        ) : attendances.length === 0 ? (
          <div className="p-12 text-center text-slate-500">
            <ClipboardCheck className="w-12 h-12 mx-auto text-slate-300 mb-3" />
            <p className="font-semibold">No attendance records found for your branch.</p>
          </div>
        ) : (
...
export default function FacultyAttendancePage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-slate-500">Loading Attendance Portal...</div>}>
