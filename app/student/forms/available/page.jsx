...
export default function AvailableFormsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/forms/available?${params}`);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 animate-pulse">
          {[1,2,3,4,5,6].map(i => <div key={i} className="bg-slate-100 dark:bg-slate-800 rounded-2xl h-56" />)}
        </div>
      ) : forms.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700">
          ...
          <p className="text-base font-semibold text-slate-600 dark:text-slate-400">No forms available right now</p>
          <p className="text-sm text-slate-400 mt-1">Check back later for new forms from your department</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {forms.map(form => (
            <FormCard key={form.id} form={form} variant="student" baseHref="/student/forms" />
          ))}
        </div>
      )}
