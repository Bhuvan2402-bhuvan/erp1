...
function LoginForm() {
  ...
  useEffect(() => {
    if (urlError === 'auth-callback-failed') {
      setError('An error occurred during authentication callback. Please try again.');
    } else if (urlError === 'no-email-from-google') {
      setError('Could not retrieve an email address from your Google account.');
            ...
            <div className="mt-6">
              ...
              <div className="mt-5">
                <button
                  ...
                  className="w-full flex justify-center items-center py-2.5 px-4 border border-slate-300 dark:border-slate-600 rounded-xl shadow-sm text-sm font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {googleLoading ? (
